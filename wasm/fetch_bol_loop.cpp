...
/* emcc wasm/fetch_bol_loop.cpp  -Os -s WASM=1 -s ASSERTIONS=1 -s MODULARIZE=1 -s FETCH=1 -s -o wasm/fetch_bol_loop_mod.js */ 
...
void downloadSucceeded(emscripten_fetch_t *fetch) {
    ...
    if(indexCounter == 100){
        ...
        printf("total time to fetch x 100 = %lld ms\n", time/std::chrono::milliseconds(1));
    }
}
...
int main() {
    for(size_t i = 0; i < 100; i++)
    {
        ...
        emscripten_fetch(&attr, "./json/restCountries.json");
    }
}